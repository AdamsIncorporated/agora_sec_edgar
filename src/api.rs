...
pub fn fetch_metadata_from_server(cik_str: &str) -> Result<CompanyTickers, std::io::Error> {
    let mut stream = TcpStream::connect("data.sec.gov:80")?;

    let request = format!(
        "GET /submissions/company_tickers.json HTTP/1.1\r\n\
         Host: data.sec.gov\r\n\
